...
function Текст-Списка {
    ...
    foreach ($Товар in $Товары) {
        ...
        if ($Текущее -ne $null -and $Текущее.Цена -gt 0) {
            $Цена = "цена {0:N0} {1}" -f (Вычислить-Итого -Данные $Текущее), $Единица
        }

        $Строка = "{0}: {1}, {2}" -f `
            $Товар.Артикул, (Ссылка-Названием -Артикул $Товар.Артикул -Наименование "Ссылка на товар"), $Цена

        if ($Товар.ЦелеваяЦена -gt 0) { $Строка = $Строка + (" — цель {0:N0} {1}" -f $Товар.ЦелеваяЦена, $Единица) }

        $Строки += $Строка
    }

    return ($Строки -join "`n")
}
...
function Команда-Удалить {
    ...
    if ($Аргументы.Count -lt 2) {
        Отправить-Телеграм -Текст "Нужен артикул. Например: /remove 1422010347"
        return
    }
