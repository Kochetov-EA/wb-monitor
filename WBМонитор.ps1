...
function Прочитать-Смещение {
    ...
    $Текст = (Get-Content -Path $ФайлСмещения -Raw -Encoding UTF8).Trim()
    if ($Текст -notmatch '^\d+$') { return [long]0 }
...
function Записать-Смещение {
    ...
    Set-Content -Path $ФайлСмещения -Encoding UTF8 -Value ([string]$Смещение)
}
...
/add 1422010347 430000 — добавить артикул, второе число это целевая цена (можно не указывать)
...
function Текст-Списка {
    ...
    foreach ($Товар in $Товары) {
        ...
        $Хвост = ""
        if ($Товар.ЦелеваяЦена -gt 0) { $Хвост = " — цель {0:N0} {1}" -f $Товар.ЦелеваяЦена, $Единица }

        $Строки += "{0}. {1}{2}" -f $Товар.Артикул, $Товар.Заметка, $Хвост
    }
...
function Текст-Цен {
    ...
    foreach ($Товар in $Товары) {
        ...
        if ($Текущее -eq $null -or $Текущее.Цена -le 0) {
            $Строки += "{0} — нет в наличии" -f $Товар.Артикул
            continue
        }

        $Итого = Вычислить-Итого -Данные $Текущее
        $Строки += "{0}`n{1:N0} {2} ({3:N0} + {4:N0} дост.)" -f `
            $Текущее.Наименование, $Итого, $Единица, $Текущее.Цена, $Текущее.Доставка
    }
...
function Команда-Добавить {
    ...
    $Ответ = "Добавил: {0}" -f $Товар.Наименование
